...
const sectionRows = []; // sectionRows[si] = [bee-item-row, ...]

function renderCatalog() {
  const root = document.getElementById("catalog");
  CATALOG.forEach((sec, si) => {
    const details = document.createElement("details");
    details.dataset.section = si;
...
function recalc() {
  ...
  CATALOG.forEach((_, si) => {
    ...
    document.querySelector(`details[data-section="${si}"]`).dataset.hasItems = active > 0 ? "true" : "false";
  });
  const subStr = subtotal.toFixed(2);
  document.getElementById("live-totals").setAttribute("subtotal", subStr);
  document.getElementById("print-totals").setAttribute("subtotal", subStr);
}
...
function saveState() {
  ...
  try {
    const state = {
      ...
      sections: Array.from(document.querySelectorAll("#catalog > details")).map((d) => d.open),
    };
...
function loadState() {
  ...
  loadingState = true;
  if (state.customer) {
    if (state.customer.name) document.getElementById("cust-name").value = state.customer.name;
    if (state.customer.phone) document.getElementById("cust-phone").value = state.customer.phone;
  }
  ...
  if (Array.isArray(state.sections)) {
    document.querySelectorAll("#catalog > details").forEach((d, i) => {
...
function setAllSections(open) {
  document.querySelectorAll("#catalog > details").forEach((d) => { d.open = open; });
  saveState();
}

document.getElementById("cust-date").value = new Date().toISOString().slice(0, 10);
document.getElementById("clear-btn").addEventListener("click", clearAll);
document.getElementById("expand-all-btn").addEventListener("click", () => setAllSections(true));
document.getElementById("collapse-all-btn").addEventListener("click", () => setAllSections(false));
document.getElementById("print-btn").addEventListener("click", () => {
  ...
  document.querySelectorAll('#catalog > details[data-has-items="true"]').forEach((d) => { d.open = true; });
  window.print();
});
...
// and on details open/close.
document.getElementById("catalog").addEventListener("line-change", saveState);
document.getElementById("catalog").addEventListener("toggle", saveState, true);
